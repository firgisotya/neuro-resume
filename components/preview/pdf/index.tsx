...
import styles from "@/components/preview/Style";
  ...
  return (
    <Document language="en">
      <Page size="A4" style={styles.page}>
        ...
        {data?.educations && <Education data={data?.educations} />}
        {data?.experiences && <Experience data={data?.experiences} />}
        {data?.projects && <Projects data={data?.projects} />}

        <Section title={"skills"}>
          <Text style={{ fontSize: 11 }}>{data?.skill}</Text>
        </Section>
        {data?.certificates && <Certificaes data={data?.certificates} />}
